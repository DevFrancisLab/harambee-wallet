...
var BaseSigner = class _BaseSigner {
  constructor(ed25519PrivateKey) {
    (0, import_core.setInConwayEra)(true);
  ...
  static fromExtendedKeyHex(keyHex) {
    return new _BaseSigner(
      import_crypto.Ed25519PrivateKey.fromExtendedHex((0, import_crypto.Ed25519PrivateExtendedKeyHex)(keyHex))
    );
  }
  /**
  ...
  static fromNormalKeyHex(keyHex) {
    return new _BaseSigner(
      import_crypto.Ed25519PrivateKey.fromNormalHex((0, import_crypto.Ed25519PrivateNormalKeyHex)(keyHex))
    );
  }
   ...
   */
  async sign(data) {
    return this.ed25519PrivateKey.sign((0, import_util.HexBlob)(data)).hex();
  }
  /**
  ...
  async verify(data, signature) {
    return this.ed25519PrivateKey.toPublic().verify(
      import_crypto.Ed25519Signature.fromHex((0, import_crypto.Ed25519SignatureHex)(signature)),
      (0, import_util.HexBlob)(data)
    );
  }
};
...
var InMemoryBip32 = class _InMemoryBip32 {
  ...
  static async fromMnemonic(mnemonic, password) {
    ...
    const bip32PrivateKey = import_crypto2.Bip32PrivateKey.fromBip39Entropy(
...
var CardanoAddress = class {
  constructor(addressType, networkId, paymentCredential, stakeCredential) {
    (0, import_core2.setInConwayEra)(true);
  ...
  getEnterpriseAddressBech32() {
    const enterpriseAddress = import_core2.Cardano.EnterpriseAddress.fromCredentials(
      this.networkId === 1 ? import_core2.Cardano.NetworkId.Mainnet : import_core2.Cardano.NetworkId.Testnet,
      {
  ...
  getBaseAddressBech32() {
    ...
    const baseAddress = import_core2.Cardano.BaseAddress.fromCredentials(
      this.networkId === 1 ? import_core2.Cardano.NetworkId.Mainnet : import_core2.Cardano.NetworkId.Testnet,
      {
        hash: (0, import_crypto3.Hash28ByteBase16)(this.paymentCredential.hash),
        type: mapCredentialTypeToCredential(this.paymentCredential.type)
  ...
  getRewardAddressBech32() {
    if (!this.stakeCredential) throw new Error("No stake credential");
    const rewardAddress = import_core2.Cardano.RewardAddress.fromCredentials(
      this.networkId === 1 ? import_core2.Cardano.NetworkId.Mainnet : import_core2.Cardano.NetworkId.Testnet,
      {
        hash: (0, import_crypto3.Hash28ByteBase16)(this.stakeCredential.hash),
        type: this.stakeCredential.type
...
var mapCredentialTypeToCredential = (credentialType) => {
  switch (credentialType) {
    case 0 /* KeyHash */:
      return import_core2.Cardano.CredentialType.KeyHash;
    case 1 /* ScriptHash */:
      return import_core2.Cardano.CredentialType.ScriptHash;
  }
};
...
var toTxUnspentOutput = (utxo) => {
  const txInput = new import_core3.Serialization.TransactionInput(
    import_core3.Cardano.TransactionId(utxo.input.txHash),
    BigInt(utxo.input.outputIndex)
  );
  const txOutput = new import_core3.Serialization.TransactionOutput(
    import_core3.Cardano.Address.fromBech32(utxo.output.address),
    toValue(utxo.output.amount)
  );
  if (utxo.output.dataHash !== void 0) {
    txOutput.setDatum(
      import_core3.Serialization.Datum.fromCore((0, import_crypto4.Hash32ByteBase16)(utxo.output.dataHash))
    );
  }
  if (utxo.output.plutusData !== void 0) {
    const plutusData = import_core3.Serialization.PlutusData.fromCbor(
      (0, import_util2.HexBlob)(utxo.output.plutusData)
    );
  ...
  if (utxo.output.scriptRef !== void 0) {
    txOutput.setScriptRef(
      import_core3.Serialization.Script.fromCbor((0, import_util2.HexBlob)(utxo.output.scriptRef))
    );
  }
  return new import_core3.Serialization.TransactionUnspentOutput(txInput, txOutput);
};
...
var toValue = (assets) => {
  ...
  assets.filter((asset) => asset.unit !== "lovelace" && asset.unit !== "").forEach((asset) => {
    multiAsset.set(import_core3.Cardano.AssetId(asset.unit), BigInt(asset.quantity));
  });
  const lovelace = assets.find(
    (asset) => asset.unit === "lovelace" || asset.unit === ""
  );
  const value = new import_core3.Serialization.Value(
...
var MeshCardanoBrowserWallet = class _MeshCardanoBrowserWallet extends CardanoBrowserWallet {
  ...
  async getUtxosMesh() {
    ...
    return utxosCbor.map(
      (utxoCbor) => fromTxUnspentOutput(
        import_core4.Serialization.TransactionUnspentOutput.fromCbor((0, import_util3.HexBlob)(utxoCbor))
      )
    );
  }
  ...
  async getBalanceMesh() {
    ...
    const value = import_core4.Serialization.Value.fromCbor((0, import_util3.HexBlob)(balanceCbor));
    return fromValue(value);
  }
  async getUsedAddressesBech32() {
    const addressesHex = await this.getUsedAddresses();
    return addressesHex.map((addr) => {
      const cardanoAddr = import_core4.Cardano.Address.fromBytes((0, import_util3.HexBlob)(addr));
  ...
  async getChangeAddressBech32() {
    ...
    const cardanoAddr = import_core4.Cardano.Address.fromBytes((0, import_util3.HexBlob)(addressHex));
    return cardanoAddr.toBech32();
  }
  ...
  async signTxReturnFullTx(tx, partialSign = false) {
    ...
    const addedWitnesses = import_core4.Serialization.TransactionWitnessSet.fromCbor(
      (0, import_util3.HexBlob)(witnessCbor)
    );
    const transaction = import_core4.Serialization.Transaction.fromCbor(
      import_core4.Serialization.TxCBOR(tx)
    );
    ...
    witnessSet.setVkeys(
      import_core4.Serialization.CborSet.fromCore(
        witnessSetVkeysValues.map((vkw) => vkw.toCore()),
        import_core4.Serialization.VkeyWitness.fromCore
      )
    );
    const signedTx = new import_core4.Serialization.Transaction(
      transaction.body(),
      witnessSet,
...
var CoseSign1 = class _CoseSign1 {
  ...
  createSigStructure(externalAad = Buffer.alloc(0)) {
    let protectedSerialized = new import_cbor.CborBytes(Buffer.alloc(0));
    ...
    return Buffer.from(import_cbor.Cbor.encode(structure).toBuffer());
  }
  buildMessage(signature) {
    ...
    return Buffer.from(import_cbor.Cbor.encode(coseSign1).toBuffer());
  }
  verifySignature({
    ...
    return publicKey.verify(
      ...
      (0, import_util4.HexBlob)(Buffer.from(this.createSigStructure(externalAad)).toString("hex"))
    );
  }
  hashPayload() {
    ...
    this.payload = new import_cbor.CborBytes(hash);
  }
  getAddress() {
...
var getCoseKeyFromPublicKey = (cbor) => {
  ...
  coseKeyMap.push({
    ...
    v: new import_cbor.CborBytes(Buffer.from(cbor, "hex"))
  });
  return Buffer.from(import_cbor.Cbor.encode(new import_cbor.CborMap(coseKeyMap)).toBuffer());
};
...
var CardanoSigner = class {
  ...
  static async signTx(tx, signers, returnFullTx = false) {
    ...
    for (const signer of signers) {
      vkeyWitnesses.push([
        (0, import_crypto5.Ed25519PublicKeyHex)(await signer.getPublicKey()),
        (0, import_crypto5.Ed25519SignatureHex)(await signer.sign((0, import_util5.HexBlob)(txHash)))
      ]);
    }
    if (returnFullTx) {
  ...
  static async signData(data, addressHex, signer) {
    ...
    protectedMap.push({
      ...
      v: new import_cbor2.CborBytes(Buffer.from(addressHex, "hex"))
    });
...
var CardanoHeadlessWallet = class _CardanoHeadlessWallet {
  ...
  async getBalance() {
    ...
    const utxos = await this.fetchAccountUtxos();
    const cardanoUtxos = utxos.map((utxo) => toTxUnspentOutput(utxo));
    let total = new import_core9.Serialization.Value(0n);
  ...
  async signTx(tx, partialSign = false) {
    ...
    const transaction = import_core9.Serialization.Transaction.fromCbor(
      import_core9.Serialization.TxCBOR(tx)
    );
  ...
  async signData(addressBech32, data) {
    ...
    const address = import_core9.Cardano.Address.fromBech32(targetAddressBech32);
    if (!address) {
      throw new Error("[CardanoWallet] Invalid address");
    }
    const addressProps = address.getProps();
    let credentialHash;
    if (addressProps.type === import_core9.Cardano.AddressType.RewardKey && addressProps.delegationPart) {
...
var MeshCardanoHeadlessWallet = class _MeshCardanoHeadlessWallet extends CardanoHeadlessWallet {
  ...
  async getUsedAddressesBech32() {
    ...
    return addresses.map((addr) => {
      const cardanoAddr = import_core10.Cardano.Address.fromBytes((0, import_util6.HexBlob)(addr));
      return cardanoAddr.toBech32();
    });
  }
  /**
  ...
  async getChangeAddressBech32() {
    ...
    const cardanoAddr = import_core10.Cardano.Address.fromBytes((0, import_util6.HexBlob)(address));
    return cardanoAddr.toBech32();
  }
  /**
   * Get the reward address for the wallet.
  ...
  async signTxReturnFullTx(tx, partialSign = false) {
    const witnessCbor = await this.signTx(tx, partialSign);
    const addedWitnesses = import_core10.Serialization.TransactionWitnessSet.fromCbor(
      (0, import_util6.HexBlob)(witnessCbor)
    );
    const transaction = import_core10.Serialization.Transaction.fromCbor(
      import_core10.Serialization.TxCBOR(tx)
    );
    let witnessSet = transaction.witnessSet();
    let witnessSetVkeys = witnessSet.vkeys();
    let witnessSetVkeysValues = witnessSetVkeys ? [
    ...
    witnessSet.setVkeys(
      import_core10.Serialization.CborSet.fromCore(
        witnessSetVkeysValues.map((vkw) => vkw.toCore()),
        import_core10.Serialization.VkeyWitness.fromCore
      )
    );
    const signedTx = new import_core10.Serialization.Transaction(
      transaction.body(),
      witnessSet,
      transaction.auxiliaryData()
    );
