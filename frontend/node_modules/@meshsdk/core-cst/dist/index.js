...
// src/index.ts
import { Cardano as Cardano7, Serialization as Serialization9 } from "@cardano-sdk/core";
...
var HexBlob = CardanoHexBlob;
var TxCBOR = Serialization.TxCBOR;
...
var BootstrapWitness = Serialization.BootstrapWitness;
...
import { blake2b as blake2b3 } from "blakejs";
var CoseSign1 = class _CoseSign1 {
  ...
  constructor(payload) {
    ...
    if (!this.unProtectedMap.map.find((value) => {
      return import_json_bigint.default.stringify(value.k.toRawObj()) === import_json_bigint.default.stringify({
        text: "hashed"
      });
    })) {
  ...
  createSigStructure(externalAad = Buffer2.alloc(0)) {
    let protectedSerialized = new CborBytes(Buffer2.alloc(0));
    ...
    return Buffer2.from(Cbor.encode(structure).toBuffer());
  }
  buildMessage(signature) {
    ...
    return Buffer2.from(Cbor.encode(coseSign1).toBuffer());
  }
  verifySignature({
    ...
    return publicKey.verify(
      ...
      HexBlob(
        Buffer2.from(this.createSigStructure(externalAad)).toString("hex")
      )
    );
  }
  hashPayload() {
    ...
    const hashedIndex = this.unProtectedMap.map.findIndex((value) => {
      return import_json_bigint.default.stringify(value.k) === import_json_bigint.default.stringify(new CborText("hashed"));
    });
    const hashed = this.unProtectedMap.map[hashedIndex];
    if (hashed && import_json_bigint.default.stringify(hashed.v) === import_json_bigint.default.stringify(new CborSimple(true)))
      throw Error("Payload already hashed");
    if (hashed && import_json_bigint.default.stringify(hashed.v) === import_json_bigint.default.stringify(new CborSimple(true)) != false)
      throw Error("Invalid unprotected map");
    this.unProtectedMap.map.splice(hashedIndex, 1);
    const hash2 = blake2b3(this.payload.bytes, void 0, 24);
    this.payload = new CborBytes(hash2);
  }
  getAddress() {
    ...
    return Buffer2.from(address.v.bytes);
  }
  getPublicKey() {
    ...
    return Buffer2.from(publicKey.v.bytes);
  }
  getSignature() {
    return this.signature ? Buffer2.from(this.signature.bytes) : this.signature;
  }
  getPayload() {
    return this.payload ? Buffer2.from(this.payload.bytes) : this.payload;
  }
};
var getPublicKeyFromCoseKey = (cbor) => {
  const decodedCoseKey = Cbor.parse(cbor).toRawObj();
  const publicKeyEntry = decodedCoseKey.map.find((value) => {
    return import_json_bigint.default.stringify(value.k) === import_json_bigint.default.stringify({ neg: BigInt(-2) });
  });
  if (publicKeyEntry) {
    return Buffer2.from(publicKeyEntry.v.bytes);
  }
...
var getCoseKeyFromPublicKey = (cbor) => {
  ...
  coseKeyMap.push({
    k: new CborNegInt(-2),
    v: new CborBytes(Buffer2.from(cbor, "hex"))
  });
  return Buffer2.from(Cbor.encode(new CborMap(coseKeyMap)).toBuffer());
};
...
import { isHexString as isHexString2, stringToHex as stringToHex3 } from "@meshsdk/common";
var signData = (data, signer) => {
  ...
  protectedMap.push({ k: new CborUInt2(4), v: new CborBytes2(publicKey) });
  protectedMap.push({
    k: new CborText2("address"),
    v: new CborBytes2(Buffer.from(signer.address.toBytes(), "hex"))
  });
...
import { Cardano as Cardano3 } from "@cardano-sdk/core";
...
var buildDRepID = (dRepKey, networkId = NetworkId.Testnet, addressType = AddressType.EnterpriseKey) => {
  ...
  return HexBlob2.toTypedBech32(
    "drep",
    HexBlob2.fromBytes(paymentAddress)
  );
};
...
var fromBuilderToPlutusData = (data) => {
  if (data.type === "Mesh") {
    ...
    return PlutusData.fromCbor(HexBlob3(data.content));
...
var datumCborToJson = (datumCbor) => {
  const parsedDatum = PlutusData.fromCbor(HexBlob3(datumCbor));
...
var deserializePlutusScript = (plutusScript, version) => {
  switch (version) {
    case "V1":
      return PlutusV1Script.fromCbor(HexBlob4(plutusScript));
    case "V2":
      return PlutusV2Script.fromCbor(HexBlob4(plutusScript));
    case "V3":
      return PlutusV3Script.fromCbor(HexBlob4(plutusScript));
    default:
      throw new Error("Invalid Plutus script version");
  }
};
var deserializeNativeScript = (nativeScript) => NativeScript.fromCbor(HexBlob4(nativeScript));
var deserializeScriptHash = (scriptHash) => ScriptHash.fromEd25519KeyHashHex(Ed25519KeyHashHex3(scriptHash));
var deserializeScriptRef = (scriptRef) => Script.fromCbor(HexBlob4(scriptRef));
...
var fromScriptRef = (scriptRef) => {
  const script = Script.fromCbor(HexBlob5(scriptRef));
...
var serializePlutusAddressToBech32 = (plutusHex, networkId = 0) => {
  const cardanoPlutusData = PlutusData.fromCbor(HexBlob6(plutusHex));
...
var v2ScriptToBech32 = (scriptCbor, stakeCredential, networkId = 0, isScriptStakeCredential = false) => scriptHashToBech32(
  Script.fromCbor(HexBlob6(scriptCbor)).hash().toString(),
  stakeCredential,
  networkId,
...
// src/utils/proposal.ts
import { Cardano as Cardano2, Serialization as Serialization4 } from "@cardano-sdk/core";
...
var toCardanoGovernanceActionId = (govActionId) => {
  ...
  return {
    id: Cardano2.TransactionId(govActionId.transactionId),
...
var toCardanoProtocolParamUpdate = (update) => {
  ...
  if (update.minCommitteeSize !== void 0)
    result.minCommitteeSize = Number(update.minCommitteeSize);
  if (update.committeeTermLimit !== void 0)
    result.committeeTermLimit = Cardano2.EpochNo(update.committeeTermLimit);
  if (update.govActionValidityPeriod !== void 0)
    result.governanceActionValidityPeriod = Cardano2.EpochNo(
      update.govActionValidityPeriod
    );
  if (update.govActionDeposit !== void 0)
    result.governanceActionDeposit = Number(update.govActionDeposit);
  if (update.drepDeposit !== void 0)
    result.dRepDeposit = Number(update.drepDeposit);
  if (update.drepInactivityPeriod !== void 0)
    result.dRepInactivityPeriod = Cardano2.EpochNo(update.drepInactivityPeriod);
  if (update.refScriptCostPerByte !== void 0)
...
var toCardanoCommittee = (committee) => {
  ...
  for (const member of committee.members) {
    members.push({
      ...
      epoch: Cardano2.EpochNo(member.termLimit)
    });
  }
  return {
    members,
...
var toCardanoProposalProcedure = (governanceAction, anchor, rewardAccount, deposit) => {
  ...
  switch (governanceAction.kind) {
    case "ParameterChangeAction": {
      ...
      const parameterChangeAction = {
        __typename: Cardano2.GovernanceActionType.parameter_change_action,
    ...
    case "HardForkInitiationAction": {
      ...
      const hardForkAction = {
        __typename: Cardano2.GovernanceActionType.hard_fork_initiation_action,
        governanceActionId: toCardanoGovernanceActionId(action.govActionId),
        protocolVersion: {
    ...
    case "TreasuryWithdrawalsAction": {
      ...
      const treasuryAction = {
        __typename: Cardano2.GovernanceActionType.treasury_withdrawals_action,
    ...
    case "NoConfidenceAction": {
      ...
      const noConfidenceAction = {
        __typename: Cardano2.GovernanceActionType.no_confidence,
        governanceActionId: toCardanoGovernanceActionId(action.govActionId)
      };
      coreProposal = {
        anchor: cardanoAnchor,
    ...
    case "UpdateCommitteeAction": {
      ...
      const updateCommitteeAction = {
        __typename: Cardano2.GovernanceActionType.update_committee,
        governanceActionId: toCardanoGovernanceActionId(action.govActionId),
    ...
    case "NewConstitutionAction": {
      ...
      const newConstitutionAction = {
        __typename: Cardano2.GovernanceActionType.new_constitution,
        governanceActionId: toCardanoGovernanceActionId(action.govActionId),
    ...
    case "InfoAction": {
      const infoAction = {
        __typename: Cardano2.GovernanceActionType.info_action
      };
      coreProposal = {
        anchor: cardanoAnchor,
        deposit,
  ...
  return Serialization4.ProposalProcedure.fromCore(coreProposal);
};
...
var resolveNativeScriptAddress = (script, networkId = 0) => {
  ...
  const enterpriseAddress = EnterpriseAddress.fromCredentials(networkId, {
    ...
    type: Cardano3.CredentialType.ScriptHash
  });
...
var resolveTxHash = (txHex) => {
  ...
  const hash2 = blake2b5(blake2b5.BYTES).update(hexToBytes(txBody.toCbor())).digest();
  return Cardano3.TransactionId.fromHexBlob(HexBlob7.fromBytes(hash2)).toString();
};
...
import { Serialization as Serialization8, setInConwayEra } from "@cardano-sdk/core";
import { HexBlob as HexBlob9 } from "@cardano-sdk/util";
import {
  Cbor as Cbor2,
  CborArray as CborArray2,
  CborBytes as CborBytes3,
  CborString,
  CborTag,
  CborUInt as CborUInt3
} from "@harmoniclabs/cbor";
...
var toCardanoCert = (cert) => {
  switch (cert.type) {
    case "RegisterPool": {
      ...
      for (const relay of cert.poolParams.relays) {
        switch (relay.type) {
          case "SingleHostAddr": {
            relays.push(
              Serialization5.Relay.newSingleHostAddr(
                new Serialization5.SingleHostAddr(
                  relay.port,
                  relay.IPV4,
                  relay.IPV6
                )
          ...
          case "SingleHostName": {
            relays.push(
              Serialization5.Relay.newSingleHostName(
                new Serialization5.SingleHostName(relay.domainName, relay.port)
              )
            );
            break;
          }
          case "MultiHostName": {
            relays.push(
              Serialization5.Relay.newMultiHostName(
                new Serialization5.MultiHostName(relay.domainName)
              )
            );
            break;
          }
      ...
      let poolOwners = Serialization5.CborSet.fromCore([], Serialization5.Hash.fromCore);
      let poolOwnersValues = [...poolOwners.values()];
      for (const poolOwner of cert.poolParams.owners) {
        poolOwnersValues.push(
          Serialization5.Hash.fromCore(Ed25519KeyHashHex2(poolOwner))
        );
      }
      poolOwners.setValues(poolOwnersValues);
      const rewardAddress = Cardano4.RewardAddress.fromAddress(
        Cardano4.Address.fromBech32(cert.poolParams.rewardAddress)
      );
    ...
    case "RegisterStake": {
      const rewardAddress = Cardano4.RewardAddress.fromAddress(
        Cardano4.Address.fromBech32(cert.stakeKeyAddress)
      );
      if (rewardAddress === void 0) {
        throw new Error(
          "Error parsing reward address, it is expected to be in bech32 format"
      ...
      return Certificate.newStakeRegistration(
        new Serialization5.StakeRegistration(
    ...
    case "DelegateStake": {
      ...
      return Certificate.newStakeDelegation(
        new Serialization5.StakeDelegation(
          rewardAddress.getPaymentCredential(),
          cert.poolId.startsWith("pool1") ? Cardano4.PoolId.toKeyHash(Cardano4.PoolId(cert.poolId)) : Ed25519KeyHashHex2(cert.poolId)
        )
      );
    }
    case "DeregisterStake": {
      ...
      return Certificate.newStakeDeregistration(
        new Serialization5.StakeDeregistration(
          rewardAddress.getPaymentCredential()
        )
      );
    }
    case "RetirePool": {
      return Certificate.newPoolRetirement(
        new Serialization5.PoolRetirement(
          cert.poolId.startsWith("pool1") ? Cardano4.PoolId.toKeyHash(Cardano4.PoolId(cert.poolId)) : Ed25519KeyHashHex2(cert.poolId),
          Cardano4.EpochNo(cert.epoch)
        )
      );
    }
    case "VoteDelegation": {
      ...
      if (cert.drep.dRepId !== void 0) {
        return Certificate.newVoteDelegationCert(
          new Serialization5.VoteDelegation(
            rewardAddress.getPaymentCredential(),
        ...
        return Certificate.newVoteDelegationCert(
          new Serialization5.VoteDelegation(
            rewardAddress.getPaymentCredential(),
            Serialization5.DRep.newAlwaysAbstain()
          )
        );
        ...
        return Certificate.newVoteDelegationCert(
          new Serialization5.VoteDelegation(
            rewardAddress.getPaymentCredential(),
            Serialization5.DRep.newAlwaysNoConfidence()
          )
        );
      } else {
    ...
    case "StakeAndVoteDelegation": {
      ...
      if (cert.drep.dRepId !== void 0) {
        return Certificate.newStakeVoteDelegationCert(
          new Serialization5.StakeVoteDelegation(
            rewardAddress.getPaymentCredential(),
            toDRep(cert.drep.dRepId),
    ...
    case "StakeRegistrationAndDelegation": {
      ...
      return Certificate.newStakeRegistrationDelegationCert(
        new Serialization5.StakeRegistrationDelegation(
          rewardAddress.getPaymentCredential(),
          BigInt(cert.coin),
          Ed25519KeyHashHex2(cert.poolKeyHash)
        )
    ...
    case "VoteRegistrationAndDelegation": {
      ...
      if (cert.drep.dRepId !== void 0) {
        return Certificate.newVoteRegistrationDelegationCert(
          new Serialization5.VoteRegistrationDelegation(
            rewardAddress.getPaymentCredential(),
            BigInt(cert.coin),
            toDRep(cert.drep.dRepId)
          )
    ...
    case "CommitteeHotAuth": {
      const hotCred = Cardano4.Address.fromBech32(
        cert.committeeHotKeyAddress
      ).getProps().paymentPart;
      const coldCred = Cardano4.Address.fromBech32(
      ...
      return Certificate.newAuthCommitteeHotCert(
        new Serialization5.AuthCommitteeHot(coldCred, hotCred)
      );
    }
    ...
    case "DRepRegistration": {
      ...
      if (Cardano4.isDRepCredential(coreDRep)) {
        return Certificate.newRegisterDelegateRepresentativeCert(
          new Serialization5.RegisterDelegateRepresentative(
            coreDRep,
            BigInt(cert.coin),
            anchor
          )
    ...
    case "DRepDeregistration": {
      ...
      if (Cardano4.isDRepCredential(coreDRep)) {
        return Certificate.newUnregisterDelegateRepresentativeCert(
          new Serialization5.UnregisterDelegateRepresentative(
            coreDRep,
            BigInt(cert.coin)
          )
        );
    ...
    case "DRepUpdate": {
      ...
      if (Cardano4.isDRepCredential(coreDRep)) {
        return Certificate.newUpdateDelegateRepresentativeCert(
          new Serialization5.UpdateDelegateRepresentative(coreDRep, anchor)
        );
      } else {
        throw new Error("DRepId must be a Credential");
      }
...
var hashScriptData = (costModels, redemeers, datums) => {
  const writer = new Serialization6.CborWriter();
...
// src/utils/vote.ts
import { Cardano as Cardano6, Serialization as Serialization7 } from "@cardano-sdk/core";
var toCardanoVoter = (voter) => {
  switch (voter.type) {
    case "ConstitutionalCommittee": {
      switch (voter.hotCred.type) {
        case "KeyHash": {
          return Serialization7.Voter.newConstitutionalCommitteeHotKey({
            type: 0,
    ...
    case "DRep": {
      ...
      if (cardanoDrep.toKeyHash() !== void 0) {
        return Serialization7.Voter.newDrep({
          type: 0,
    ...
    case "StakingPool": {
      return Serialization7.Voter.newStakingPool(
...
var toCardanoVotingProcedure = (votingProcedure) => {
  return new Serialization7.VotingProcedure(
    toCardanoVoteKind(votingProcedure.voteKind),
    votingProcedure.anchor ? toCardanoAnchor2(votingProcedure.anchor) : void 0
  );
};
var toCardanoAnchor2 = (anchor) => {
  return new Serialization7.Anchor(
    anchor.anchorUrl,
...
var toCardanoGovernanceActionId2 = (govActionId) => {
  return new Serialization7.GovernanceActionId(
    Cardano6.TransactionId(govActionId.txHash),
...
var CardanoSDKSerializer = class {
    ...
    script: {
      ...
      deserializePlutusScript: function(script) {
        ...
        switch (script.version) {
          case "V1": {
            cardanoPlutusScript = new PlutusV1Script(HexBlob9(script.code));
            break;
          }
          case "V2": {
            cardanoPlutusScript = new PlutusV2Script(HexBlob9(script.code));
            break;
          }
          case "V3": {
            cardanoPlutusScript = new PlutusV3Script(HexBlob9(script.code));
            break;
          }
        }
        return {
  ...
  resolver = {
    keys: {
      ...
      resolvePrivateKey: function(words) {
        const buildBip32PrivateKey2 = (entropy2, password = "") => {
          return Bip32PrivateKey2.fromBip39Entropy(
            Buffer3.from(toBytes5(entropy2)),
            fromUTF82(password)
          );
        };
    ...
    tx: {
      resolveTxHash: function(txHex) {
        return Transaction.fromCbor(Serialization8.TxCBOR(txHex)).getId();
      }
    },
    data: {
    ...
    script: {
      resolveScriptRef: function(script) {
        if ("code" in script) {
          ...
          return Cbor2.encode(taggedScript).toString();
        } else {
          const nativeScript = toNativeScript(script);
          let taggedScript = new CborTag(
            24,
  ...
  addSigningKeys = (txHex, signingKeys) => {
    let cardanoTx = Transaction.fromCbor(Serialization8.TxCBOR(txHex));
    ...
    for (let i = 0; i < signingKeys.length; i++) {
      ...
      if (keyHex) {
        ...
        const signature = cardanoSigner.sign(HexBlob9(cardanoTx.getId()));
    ...
    currentWitnessSet.setVkeys(
      Serialization8.CborSet.fromCore(
        ...
        case "V1": {
          cardanoOutput.setScriptRef(
            Script.newPlutusV1Script(
              PlutusV1Script.fromCbor(HexBlob9(output.referenceScript.code))
            )
          );
          break;
        }
        case "V2": {
          cardanoOutput.setScriptRef(
            Script.newPlutusV2Script(
              PlutusV2Script.fromCbor(HexBlob9(output.referenceScript.code))
            )
          );
          break;
        }
        case "V3": {
          cardanoOutput.setScriptRef(
            Script.newPlutusV3Script(
              PlutusV3Script.fromCbor(HexBlob9(output.referenceScript.code))
            )
          );
          break;
        }
        default: {
          cardanoOutput.setScriptRef(
            Script.newNativeScript(
              NativeScript.fromCbor(HexBlob9(output.referenceScript.code))
            )
          );
          break;
        }
...
var CardanoSDKSerializerCore = class {
  ...
  constructor(protocolParams) {
    ...
    this.txBody = new TransactionBody(
      Serialization8.CborSet.fromCore([], TransactionInput.fromCore),
      [],
      BigInt(0),
      void 0
    );
      ...
      this.scriptsProvided.add(
        Script.newNativeScript(
          NativeScript.fromCbor(
            HexBlob9(currentTxIn.simpleScriptTxIn.scriptSource.scriptCode)
          )
        ).toCbor()
      );
  ...
  toCardanoOutput = (output) => {
    ...
    if (output.datum?.type === "Hash") {
      ...
      const currentWitnessDatum = this.txWitnessSet.plutusData() ?? Serialization8.CborSet.fromCore([], Serialization8.PlutusData.fromCore);
  ...
  addReferenceInput = (refInput) => {
    let referenceInputs = this.txBody.referenceInputs() ?? Serialization8.CborSet.fromCore([], TransactionInput.fromCore);
  ...
  addMint = (mint) => {
    ...
    if (mint.type === "Native") {
      ...
      if (nativeScriptSource.type === "Provided") {
        this.scriptsProvided.add(
          Script.newNativeScript(
            NativeScript.fromCbor(HexBlob9(nativeScriptSource.scriptCode))
          ).toCbor()
        );
  ...
  addCert = (cert, index) => {
    const currentCerts = this.txBody.certs() ?? Serialization8.CborSet.fromCore([], Serialization8.Certificate.fromCore);
  ...
  addCollateralInput = (collateral) => {
    ...
    const collateralInputs = this.txBody.collateral() ?? Serialization8.CborSet.fromCore([], TransactionInput.fromCore);
  ...
  addAllRequiredSignatures = (requiredSignatures) => {
    const requiredSigners = this.txBody.requiredSigners() ?? Serialization8.CborSet.fromCore([], Serialization8.Hash.fromCore);
    let requiredSignerValues = [...requiredSigners.values()];
    for (const requiredSigner of requiredSignatures) {
      requiredSignerValues.push(
        Serialization8.Hash.fromCore(Ed25519KeyHashHex2(requiredSigner))
      );
    }
  ...
  addMetadata = (metadata) => {
    this.txAuxilliaryData.setMetadata(
      new Serialization8.GeneralTransactionMetadata(
  ...
  buildWitnessSet = () => {
    ...
    let v3Scripts = this.txWitnessSet.plutusV3Scripts() ?? Serialization8.CborSet.fromCore([], PlutusV3Script.fromCore);
    this.scriptsProvided.forEach((scriptHex) => {
      const script = Script.fromCbor(HexBlob9(scriptHex));
    ...
    let datums = this.txWitnessSet.plutusData() ?? Serialization8.CborSet.fromCore([], PlutusData.fromCore);
  ...
  addProvidedPlutusScript = (script) => {
    switch (script.version) {
      case "V1": {
        this.scriptsProvided.add(
          Script.newPlutusV1Script(
            PlutusV1Script.fromCbor(HexBlob9(script.code))
          ).toCbor()
        );
        this.usedLanguages[PlutusLanguageVersion.V1] = true;
        break;
      ...
      case "V2": {
        this.scriptsProvided.add(
          Script.newPlutusV2Script(
            PlutusV2Script.fromCbor(HexBlob9(script.code))
          ).toCbor()
        );
        this.usedLanguages[PlutusLanguageVersion.V2] = true;
        break;
      ...
      case "V3": {
        this.scriptsProvided.add(
          Script.newPlutusV3Script(
            PlutusV3Script.fromCbor(HexBlob9(script.code))
          ).toCbor()
        );
        this.usedLanguages[PlutusLanguageVersion.V3] = true;
        break;
  ...
  addBasicVote = (basicVote) => {
    const votes = this.txBody.votingProcedures() ?? Serialization8.VotingProcedures.fromCore([]);
    votes.insert(
      toCardanoVoter(basicVote.vote.voter),
      toCardanoGovernanceActionId2(basicVote.vote.govActionId),
  ...
  addBasicProposal = (proposal) => {
    ...
    const proposalProcedures = currentProcedures ? [...currentProcedures.values()] : [];
    ...
    this.txBody.setProposalProcedures(
      CborSet.fromCore(
        ...
        Serialization8.ProposalProcedure.fromCore
      )
    );
  };
  ...
  serializeByronAttributes = (attributes) => {
    ...
    if (attributes.derivationPath) {
      writer.writeInt(1);
      const encodedPathCbor = new CborWriter().writeByteString(Buffer3.from(attributes.derivationPath, "hex")).encode();
...
var getPurePlutusBytes = (plutusScript) => {
  ...
  try {
    while (unwrappedScript.length >= 3 && length != unwrappedScript.length) {
      ...
      const cbor = Cbor3.parse(unwrappedScript);
      if (cbor instanceof CborBytes4) {
...
var applyCborEncoding = (plutusScript) => {
  return Cbor3.encode(new CborBytes4(plutusScript)).toBuffer();
};
...
export {
  ...
  ScriptHash,
  ScriptPubkey,
  Serialization9 as Serialization,
  Slot,
