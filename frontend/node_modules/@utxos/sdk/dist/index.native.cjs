...
__export(index_native_exports, {
  ...
  SparkWallet: () => import_spark_sdk6.SparkWallet,
  ...
  crypto: () => crypto2,
  decodeBech32mTokenIdentifier: () => import_spark_sdk6.decodeBech32mTokenIdentifier,
  decodeSparkAddress: () => import_spark_sdk6.decodeSparkAddress,
  decryptWithCipher: () => decryptWithCipher,
  decryptWithPrivateKey: () => decryptWithPrivateKey,
  encodeBech32mTokenIdentifier: () => import_spark_sdk6.encodeBech32mTokenIdentifier,
  ...
  getCrypto: () => getCrypto,
  getNetworkFromBech32mTokenIdentifier: () => import_spark_sdk6.getNetworkFromBech32mTokenIdentifier,
  getNetworkFromSparkAddress: () => import_spark_sdk6.getNetworkFromSparkAddress,
  ...
  isValidSparkAddress: () => import_spark_sdk6.isValidSparkAddress,
...
var cryptoAdapter = {
  ...
  async hmacSign(algorithm, key, data) {
    ...
    const signature = await crypto3.subtle.sign("HMAC", cryptoKey, data);
    return new Uint8Array(signature);
  },
  async encrypt(data, key, iv) {
    const crypto3 = getCrypto2();
    const cryptoKey = await crypto3.subtle.importKey("raw", key, "AES-GCM", false, ["encrypt"]);
    const encrypted = await crypto3.subtle.encrypt({ name: "AES-GCM", iv }, cryptoKey, data);
    return new Uint8Array(encrypted);
  },
  async decrypt(data, key, iv) {
    const crypto3 = getCrypto2();
    const cryptoKey = await crypto3.subtle.importKey("raw", key, "AES-GCM", false, ["decrypt"]);
    const decrypted = await crypto3.subtle.decrypt({ name: "AES-GCM", iv }, cryptoKey, data);
    return new Uint8Array(decrypted);
  },
  async generateKeyPair(algorithm = "ECDH") {
    const crypto3 = getCrypto2();
    const keyPair = await crypto3.subtle.generateKey(
      { name: algorithm, namedCurve: "P-256" },
      true,
      algorithm === "ECDH" ? ["deriveBits"] : ["sign", "verify"]
    );
    const publicKey = await crypto3.subtle.exportKey("raw", keyPair.publicKey);
    const privateKey = await crypto3.subtle.exportKey("pkcs8", keyPair.privateKey);
    return {
  ...
  async deriveSharedSecret(privateKey, publicKey) {
    const crypto3 = getCrypto2();
    const privKey = await crypto3.subtle.importKey(
      "pkcs8",
      privateKey,
    ...
    const pubKey = await crypto3.subtle.importKey(
      "raw",
      publicKey,
      { name: "ECDH", namedCurve: "P-256" },
      false,
    ...
    const sharedSecret = await crypto3.subtle.deriveBits(
  ...
  async importKey(keyData, algorithm, usages) {
    const crypto3 = getCrypto2();
    return crypto3.subtle.importKey("raw", keyData, algorithm, false, usages);
  },
  getSubtleCrypto() {
...
function getHmacAlgorithm2(algorithm) {
  const normalized = algorithm.toUpperCase().replace(/SHA-?(\d+)/i, "SHA-$1");
  return {
    name: "HMAC",
    hash: { name: normalized }
...
var cryptoAdapter2 = {
   ...
   */
  async hmacSign(algorithm, key, data) {
    const hmacAlgorithm = getHmacAlgorithm2(algorithm);
...
var browserAdapters = {
  crypto: cryptoAdapter2,
  storage: storageAdapter2,
  linking: linkingAdapter2,
  encoding: encodingAdapter2
};
...
async function combineShardsBuildWallet(networkId, keyShard1, keyShard2, bitcoinProvider, fetcher) {
  ...
  const { wallet: sparkWallet } = await import_spark_sdk.SparkWallet.initialize({
...
async function clientGenerateWallet(deviceShardEncryptionKey, recoveryShardEncryptionKey) {
  ...
  const { wallet: sparkMainnetWallet } = await import_spark_sdk2.SparkWallet.initialize({
    mnemonicOrSeed: mnemonic,
    options: {
      network: "MAINNET"
    }
  });
  const { wallet: sparkRegtestWallet } = await import_spark_sdk2.SparkWallet.initialize({
    mnemonicOrSeed: mnemonic,
    options: {
      network: "REGTEST"
    }
...
function extractIdentityPublicKey(sparkAddress) {
  try {
    const network = (0, import_spark_sdk3.getNetworkFromSparkAddress)(
      sparkAddress
    );
    const decoded = (0, import_spark_sdk3.decodeSparkAddress)(sparkAddress, network);
...
function getSparkAddressFromPubkey(identityPublicKey, network) {
  const identityPublicKeyBytes = hexToBytes2(identityPublicKey);
...
function convertLegacyToNewFormat(legacyAddress) {
  ...
  const { words } = (0, import_spark_sdk3.bech32mDecode)(legacyAddress);
  return bech32m.encode(NEW_PREFIXES[network], words, 1024);
}
...
var Web3SparkWallet = class {
  ...
  constructor(options) {
    ...
    this._axiosInstance = import_axios2.default.create({
