...
function getBitcoinAddressFromPubkey(pubKeyHash, networkId) {
  return resolveAddress(pubKeyHash, networkId === 1 ? "mainnet" : "testnet").address;
}
...
import { MeshCardanoHeadlessWallet as MeshCardanoHeadlessWallet2 } from "@meshsdk/wallet";
import { generateMnemonic } from "@meshsdk/common";
import { deserializeBech32Address } from "@meshsdk/core-cst";
import { EmbeddedWallet as EmbeddedWallet2 } from "@meshsdk/bitcoin";
import { SparkWallet as SparkWallet2 } from "@buildonspark/spark-sdk";
async function clientGenerateWallet(deviceShardEncryptionKey, recoveryShardEncryptionKey) {
  ...
  const bitcoinTestnetWallet = new EmbeddedWallet2({
    network: "Testnet",
    key: {
      type: "mnemonic",
      words: mnemonic.split(" ")
  ...
  const bitcoinMainnetWallet = new EmbeddedWallet2({
    network: "Mainnet",
    key: {
      type: "mnemonic",
      words: mnemonic.split(" ")
  ...
  const cardanoWallet = await MeshCardanoHeadlessWallet2.fromMnemonic({
    mnemonic: mnemonic.split(" "),
    networkId: 1,
    walletAddressType: 1
  });
  ...
  const { wallet: sparkMainnetWallet } = await SparkWallet2.initialize({
    mnemonicOrSeed: mnemonic,
    options: {
      network: "MAINNET"
    }
  });
  const { wallet: sparkRegtestWallet } = await SparkWallet2.initialize({
    mnemonicOrSeed: mnemonic,
    options: {
      network: "REGTEST"
    }
...
import { MeshCardanoHeadlessWallet as MeshCardanoHeadlessWallet3 } from "@meshsdk/wallet";
var CardanoWalletDeveloperControlled = class {
  ...
  async getWallet(walletId, decryptKey = false) {
    ...
    if (status === 200) {
      ...
      const wallet = await MeshCardanoHeadlessWallet3.fromMnemonic({
        mnemonic: mnemonic.split(" "),
        networkId,
        walletAddressType: 1
      });
...
import { MeshCardanoHeadlessWallet as MeshCardanoHeadlessWallet4 } from "@meshsdk/wallet";
import { generateMnemonic as generateMnemonic2 } from "@meshsdk/common";
...
var WalletDeveloperControlled = class {
  ...
  async createWallet(options = {}) {
    ...
    const mnemonic = (await generateMnemonic2(256)).split(" ");
    const encryptedKey = await encryptWithPublicKey({
      publicKey: project.publicKey,
      data: mnemonic.join(" ")
    });
    const cardanoWallet = await MeshCardanoHeadlessWallet4.fromMnemonic({
      mnemonic,
      networkId,
      walletAddressType: 1
    });
    ...
    const { pubKeyHash, stakeCredentialHash } = deserializeBech32Address2(
      baseAddress
    );
...
import { IssuerSparkWallet as IssuerSparkWallet2 } from "@buildonspark/issuer-sdk";
...
var TokenizationSpark = class {
  ...
  async initWalletByWalletId(walletId) {
    ...
    const { wallet } = await IssuerSparkWallet2.initialize({
      mnemonicOrSeed: mnemonic,
      options: {
...
import { MeshCardanoHeadlessWallet as MeshCardanoHeadlessWallet5 } from "@meshsdk/wallet";
var TokenizationCardano = class {
  ...
  async initWalletByWalletId(walletId) {
    ...
    const networkId = this.sdk.network === "mainnet" ? 1 : 0;
    const wallet = await MeshCardanoHeadlessWallet5.fromMnemonic({
      mnemonic: mnemonic.split(" "),
      networkId,
      walletAddressType: 1
    });
...
import { MeshCardanoHeadlessWallet as MeshCardanoHeadlessWallet6 } from "@meshsdk/wallet";
import {
  EmbeddedWallet as EmbeddedWallet3
...
var Web3Wallet = class _Web3Wallet {
  ...
  static async initBitcoinWallet(options) {
    const bitcoinWallet = new EmbeddedWallet3({
  ...
  static async initCardanoWallet(options) {
    ...
    const cardanoWallet = await MeshCardanoHeadlessWallet6.fromCredentialSources({
...
import {
  isValidSparkAddress,
  decodeSparkAddress as decodeSparkAddress2,
  getNetworkFromSparkAddress as getNetworkFromSparkAddress2,
  encodeBech32mTokenIdentifier as encodeBech32mTokenIdentifier2,
  decodeBech32mTokenIdentifier as decodeBech32mTokenIdentifier2,
  getNetworkFromBech32mTokenIdentifier,
  SparkWallet as SparkWallet3
} from "@buildonspark/spark-sdk";
...
export {
  ...
  ApiError,
  isValidSparkAddress,
  decodeSparkAddress2 as decodeSparkAddress,
  getNetworkFromSparkAddress2 as getNetworkFromSparkAddress,
  encodeBech32mTokenIdentifier2 as encodeBech32mTokenIdentifier,
  decodeBech32mTokenIdentifier2 as decodeBech32mTokenIdentifier,
  getNetworkFromBech32mTokenIdentifier,
  SparkWallet3 as SparkWallet,
  Web3SparkWallet
};
